...
func doHttpRequests(duration time.Duration) {
	...
	for {
		...
		req, _ := http.NewRequestWithContext(ctx, http.MethodPost, "http://flagd-http.flagd-performance-test:80/schema.v1.Service/ResolveString", bytes.NewReader(marshal))
